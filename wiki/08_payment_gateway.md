...
## Overview
Choice uses **Adyen** as the payment gateway for all online payments (QR, delivery, takeaway) across Europe. The pricing model is **IC++ (Interchange++)** — the standard transparent model used across the market, including on regular payment terminals.

> **Ukraine:** Adyen is not available in Ukraine. Ukrainian restaurants use **LiqPay** or **Monobank** as the payment processor instead. The IC++ pricing model applies similarly, but through these local providers.
